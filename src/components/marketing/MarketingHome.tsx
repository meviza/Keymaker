...
export function MarketingHome() {
  ...
  const navLinks = [
    ...
    { href: "#compare", label: locale === "tr" ? "Karşılaştır" : "Compare" },
  ];
    ...
    <div className="w-full min-h-screen bg-white dark:bg-cyber-black text-zinc-900 dark:text-white overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 w-full border-b border-zinc-200 dark:border-white/[0.06] bg-white/80 dark:bg-cyber-black/70 backdrop-blur-2xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          ...
          {/* Desktop nav */}
          <div className="hidden md:flex items-center gap-6 lg:gap-8 text-sm text-zinc-500 dark:text-zinc-400">
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} className="hover:text-zinc-900 dark:hover:text-white transition-colors whitespace-nowrap">{l.label}</a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="text-xs font-mono px-2 py-1 rounded border border-zinc-200 dark:border-white/10 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition"
            >
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center w-7 h-7 rounded border border-zinc-200 dark:border-white/10 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition"
            ...
            <Link href="/login" className="hidden sm:inline-flex px-3 py-1.5 text-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition whitespace-nowrap">
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="md:hidden border-t border-zinc-200 dark:border-white/[0.06] overflow-hidden"
            >
              <div className="px-4 py-4 flex flex-col gap-3">
                {navLinks.map((l) => (
                  <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)}
                    className="text-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition py-1">{l.label}</a>
                ))}
      ...
      <section className="relative z-10 pt-16 sm:pt-24 pb-16 sm:pb-20 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto text-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 mb-6 sm:mb-8 rounded-full border border-cyber-green/40 bg-cyber-green/5 text-cyber-green text-[10px] sm:text-xs font-mono tracking-widest"
          >
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-5xl md:text-7xl font-black leading-[1.05] tracking-tight mb-4 sm:mb-6 text-zinc-900 dark:text-white"
          >
            {T.hero.title1}
            <br />
          ...
          <motion.p
            ...
            className="text-base sm:text-lg text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto mb-8 sm:mb-10 leading-relaxed px-2"
          >
            {T.hero.subtitle}
          </motion.p>
          ...
          >
            ...
            <a href="#enterprise"
              className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl border border-zinc-300 dark:border-white/15 text-zinc-900 dark:text-white font-bold text-sm sm:text-base hover:bg-zinc-100 dark:hover:bg-white/5 hover:border-zinc-400 dark:hover:border-white/25 transition">
          ...
          >
            {T.stats.map((s) => (
              <motion.div key={s.label} variants={fadeUp}
                className="rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-zinc-200 dark:border-white/[0.07] bg-zinc-50/80 dark:bg-white/[0.02] hover:border-cyber-green/20 transition">
                <div className="text-2xl sm:text-3xl font-black text-cyber-green mb-1">{s.value}</div>
                <div className="text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-500 leading-tight">{s.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ── Tech Stack Bar ───────────────────────────────────────────── */}
      <section className="relative z-10 py-8 sm:py-12 border-y border-zinc-200 dark:border-white/[0.06] bg-zinc-50/80 dark:bg-white/[0.015]">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 sm:gap-4">
            {TECH_STACK.map(({ icon: Icon, label, desc }) => (
              <div key={label} className="flex flex-col items-center gap-1.5 sm:gap-2 text-center group">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-white/[0.04] flex items-center justify-center group-hover:border-cyber-green/30 group-hover:bg-cyber-green/5 transition">
                  <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-400 dark:text-zinc-500 group-hover:text-cyber-green transition" />
                </div>
                <div className="text-[10px] sm:text-[11px] font-semibold text-zinc-700 dark:text-zinc-300 leading-tight">{label}</div>
                <div className="text-[9px] sm:text-[10px] text-zinc-400 dark:text-zinc-600 hidden sm:block">{desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="products" className="relative z-10 py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <div className="text-xs font-mono text-cyber-green tracking-widest mb-3">{locale === "tr" ? "ÜRÜNLER" : "PRODUCTS"}</div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-zinc-900 dark:text-white">{T.products.title}</h2>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            >
              ...
              <div className="relative">
                ...
                <h3 className="text-xl sm:text-2xl font-black mb-2 text-zinc-900 dark:text-white">{T.products.defender.name}</h3>
                <p className="text-zinc-500 dark:text-zinc-400 mb-5 sm:mb-6 text-sm">{T.products.defender.desc}</p>
                <div className="space-y-2 sm:space-y-2.5 mb-6 sm:mb-8">
                  {T.products.defender.features.map((f) => (
                    <div key={f} className="flex items-start gap-2.5 sm:gap-3">
                      <Check className="w-4 h-4 text-cyber-green mt-0.5 shrink-0" />
                      <span className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-300">{f}</span>
                    </div>
                  ))}
                </div>
                <a href="#download" className="inline-flex items-center gap-2 text-sm font-bold text-cyber-green hover:underline group/link">
                  {locale === "tr" ? "İndir" : "Download"} <ArrowRight className="w-4 h-4 group-hover/link:translate-x-1 transition-transform" />
                </a>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="relative">
                ...
                <h3 className="text-xl sm:text-2xl font-black mb-2 text-zinc-900 dark:text-white">{T.products.sovereign.name}</h3>
                <p className="text-zinc-500 dark:text-zinc-400 mb-5 sm:mb-6 text-sm">{T.products.sovereign.desc}</p>
                <div className="space-y-2 sm:space-y-2.5 mb-6 sm:mb-8">
                  {T.products.sovereign.features.map((f) => (
                    <div key={f} className="flex items-start gap-2.5 sm:gap-3">
                ...
                <Link href="/contact" className="inline-flex items-center gap-2 text-sm font-bold text-amber-400 hover:underline group/link">
                  {T.enterprise.contact} <ArrowRight className="w-4 h-4 group-hover/link:translate-x-1 transition-transform" />
                </Link>
              </div>
            </motion.div>
          </div>
      ...
      <section id="download" className="relative z-10 py-16 sm:py-24 px-4 sm:px-6 bg-[radial-gradient(ellipse_at_center,rgba(0,255,157,0.04),transparent_70%)]">
        <div className="max-w-4xl mx-auto text-center">
          <div className="text-xs font-mono text-cyber-green tracking-widest mb-3">{locale === "tr" ? "İNDİR" : "DOWNLOAD"}</div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-3 sm:mb-4 text-zinc-900 dark:text-white">{T.download.title}</h2>
          <p className="text-zinc-500 dark:text-zinc-400 mb-8 sm:mb-12 text-sm sm:text-base">{T.download.subtitle}</p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
            {[
              { icon: Monitor, label: T.download.windows },
              { icon: Laptop, label: T.download.macos },
              { icon: Terminal, label: T.download.linux },
            ].map(({ icon: Icon, label }) => (
              <a key={label} href="#"
                className="group relative rounded-xl sm:rounded-2xl border border-zinc-200 dark:border-white/10 bg-zinc-50/80 dark:bg-white/[0.03] p-5 sm:p-6 flex flex-row sm:flex-col items-center gap-3 hover:border-cyber-green/40 hover:bg-cyber-green/[0.04] transition-all duration-300">
                <Icon className="w-8 h-8 sm:w-10 sm:h-10 text-zinc-400 dark:text-zinc-500 group-hover:text-cyber-green transition-colors shrink-0" />
                <span className="font-bold text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-white transition-colors text-sm sm:text-base">{label}</span>
                <Download className="w-4 h-4 text-zinc-300 dark:text-zinc-700 group-hover:text-cyber-green transition-colors shrink-0 sm:mt-0 ml-auto sm:ml-0" />
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 max-w-xs mx-auto mb-6 sm:mb-8">
            {[T.download.android, T.download.ios].map((label) => (
              <div key={label} className="rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-white/[0.02] p-4 flex flex-col items-center gap-2 opacity-40 cursor-not-allowed">
                <Smartphone className="w-6 h-6 sm:w-7 sm:h-7 text-zinc-400 dark:text-zinc-600" />
                <span className="text-[10px] sm:text-xs text-zinc-400 dark:text-zinc-600 text-center">{label}</span>
              </div>
            ))}
          </div>

          <p className="text-xs text-zinc-400 dark:text-zinc-700 font-mono">{T.download.note}</p>
        </div>
      </section>
      ...
      <section className="relative z-10 py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <div className="text-xs font-mono text-cyber-green tracking-widest mb-3">{locale === "tr" ? "TEKNOLOJİ" : "TECHNOLOGY"}</div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-zinc-900 dark:text-white">{T.tech.title}</h2>
          </div>
          <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
            {T.tech.items.map((item, idx) => (
              <motion.div
                ...
                className="group relative rounded-xl sm:rounded-2xl border border-zinc-200 dark:border-white/[0.07] bg-zinc-50/80 dark:bg-white/[0.02] p-5 sm:p-6 hover:border-cyber-green/25 hover:bg-cyber-green/[0.02] transition-all duration-300 overflow-hidden"
              >
                ...
                <h3 className="text-base sm:text-lg font-bold mb-2 text-zinc-900 dark:text-white">{item.title}</h3>
                <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ── Comparison ───────────────────────────────────────────────── */}
      <section id="compare" className="relative z-10 py-16 sm:py-24 px-4 sm:px-6 bg-zinc-50 dark:bg-white/[0.01]">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            <div className="text-xs font-mono text-cyber-green tracking-widest mb-3">{locale === "tr" ? "KARŞILAŞTIRMA" : "COMPARISON"}</div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-zinc-900 dark:text-white">{T.comparison.title}</h2>
          </div>

          {/* overflow-x-auto for mobile scroll */}
          <div className="rounded-2xl border border-zinc-200 dark:border-white/[0.08] overflow-x-auto">
            <table className="w-full text-xs sm:text-sm min-w-[480px]">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-white/[0.08] bg-zinc-100 dark:bg-white/[0.03]">
                  <th className="text-left py-3 sm:py-4 px-3 sm:px-6 text-zinc-500 font-medium">{T.comparison.headers[0]}</th>
                  <th className="py-3 sm:py-4 px-2 sm:px-4 text-center">
                    <span className="inline-flex items-center gap-1 sm:gap-1.5 text-cyber-green font-bold whitespace-nowrap">
                      <Shield className="w-3.5 h-3.5 shrink-0" /> {T.comparison.headers[1]}
                    </span>
                  </th>
                  <th className="py-3 sm:py-4 px-2 sm:px-4 text-center text-zinc-500 font-medium whitespace-nowrap">{T.comparison.headers[2]}</th>
                  <th className="py-3 sm:py-4 px-2 sm:px-4 text-center text-zinc-500 font-medium whitespace-nowrap">{T.comparison.headers[3]}</th>
                </tr>
              </thead>
              <tbody>
      ...
      <section id="enterprise" className="relative z-10 py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <div className="relative rounded-2xl sm:rounded-3xl border border-cyber-green/20 bg-gradient-to-b from-cyber-green/[0.06] to-transparent p-8 sm:p-12 text-center overflow-hidden">
            ...
            <div className="relative">
              <div className="text-xs font-mono text-cyber-green tracking-widest mb-3">{locale === "tr" ? "KURUMSAL" : "ENTERPRISE"}</div>
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-3 sm:mb-4 text-zinc-900 dark:text-white">{T.enterprise.title}</h2>
              <p className="text-zinc-500 dark:text-zinc-400 mb-8 sm:mb-10 max-w-xl mx-auto text-sm sm:text-base">{T.enterprise.subtitle}</p>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 max-w-xl mx-auto mb-8 sm:mb-10">
                {T.enterprise.sectors.map((s) => (
                  <span key={s} className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.04] text-xs sm:text-sm text-zinc-700 dark:text-zinc-300 text-center">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
                <Link href="/beta"
                  ...
                  {T.enterprise.cta} <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
                </Link>
                <Link href="/contact"
                  className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl border border-zinc-300 dark:border-white/20 text-zinc-900 dark:text-white font-bold text-sm sm:text-base hover:bg-zinc-100 dark:hover:bg-white/5 transition">
      ...
      {/* ── Footer ───────────────────────────────────────────────────── */}
      <footer className="relative z-10 border-t border-zinc-200 dark:border-white/[0.06] py-8 sm:py-10 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center sm:justify-between gap-4 sm:gap-6">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center sm:justify-start">
            ...
            <span className="text-zinc-400 dark:text-zinc-600 text-xs sm:text-sm">{T.footer.copyright}</span>
          </div>
          <div className="flex items-center gap-4 sm:gap-6 text-xs sm:text-sm text-zinc-400 dark:text-zinc-600 flex-wrap justify-center">
            ...
            <span className="text-cyber-green font-mono">{T.footer.location}</span>
          </div>
        </div>
      </footer>
    </div>
